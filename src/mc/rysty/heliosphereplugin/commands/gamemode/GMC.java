...
public class GMC implements CommandExecutor {
	...
	public GMC(HelioSpherePlugin plugin) {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("gmc")) {
			if (sender.hasPermission("hs.gamemode.creative")) {
				if (args.length == 0) {
					...
					if (!p.getGameMode().equals(GameMode.CREATIVE)) {
						...
						p.sendMessage(MessageUtils.chat(config.getString("GamemodeCommand.gmc")));
						return true;
					} else {
							...
							if (t == null) {
								...
								if (t.getGameMode() != GameMode.CREATIVE) {
									...
									sender.sendMessage(MessageUtils.chat(config.getString("GamemodeCommand.gm_other_message"))
											.replaceAll("<player>", tName).replaceAll("<gamemode>", "Creative"));
								} else {
